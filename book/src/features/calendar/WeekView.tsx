...
export default function WeekView() {
    ...
    // 선생님 필터 (로그인 사용자 포함)
    const { data: rawProfiles } = useProfiles(profile?.system_id)
    ...
    // 현재 로그인 계정이 치료사 목록에 무조건 포함되도록 보장
    ...
    useEffect(() => {
        if (profiles.length > 0 && selectedInstructorIds.length === 0) {
            const currentUserId = profile?.id || user?.id
            const myProfile = profiles.find((p: { id: string }) => p.id === currentUserId)
            if (myProfile) {
