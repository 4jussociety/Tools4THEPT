...
import { X, Loader2, Calendar, User, UserCheck, Search, CheckCircle, Lock, ArrowRight, Plus, Sparkles } from 'lucide-react'
import { useAuth } from '@/features/auth/AuthContext'
import { supabase } from '@/lib/supabase'
import type { Appointment, Client } from '@/types/db'
import { ChartingModal } from '@/features/charting/components/ChartingModal'
...
type ModalStep = 'TYPE_SELECT' | 'CLIENT_SEARCH' | 'QUICK_CREATE' | 'DETAIL_FORM'
...
export default function AppointmentModal({ isOpen, onClose, initialData, editingAppointment }: Props) {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 font-sans" onClick={onClose}>
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-[480px] max-h-[95vh] overflow-hidden animate-in fade-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
                ...
                {step === 'TYPE_SELECT' && (
                    <div className="p-6">
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            <button
                                onClick={() => { setValue('event_type', 'APPOINTMENT'); setStep('CLIENT_SEARCH') }}
                ...
                {step === 'QUICK_CREATE' && (
                    <div className="p-6">
                        <div className="flex justify-between items-center mb-5">
                            ...
                            <button onClick={() => setStep('CLIENT_SEARCH')} className="text-gray-400 hover:text-gray-600 transition-colors"><X className="w-5 h-5" /></button>
                        </div>
                        ...
                        <ClientForm
                            ...
                            onCancel={() => setStep('CLIENT_SEARCH')}
                        />
                    </div>
                )}
                ...
                {step === 'DETAIL_FORM' && (
                    <form onSubmit={handleSubmit(onSubmit)}>
                                    ...
                                    {eventType === 'APPOINTMENT' ? (
                                        <div className="space-y-3">
                                            <div className="bg-blue-50/50 p-2.5 rounded-xl border border-blue-100 flex items-center gap-2.5 h-[42px]">
                                                ...
                                                {!editingAppointment && (
                                                    <button type="button" onClick={() => setStep('CLIENT_SEARCH')} className="text-[10px] font-bold text-blue-600 hover:bg-blue-100 px-2 py-1 rounded-lg">변경</button>
                                                )}
                                            </div>
                        ...
                        {/* AI Charting Action Bar */}
                        {selectedClient && (
                          <div className="px-6 pb-2">
                            <button
                              type="button"
                              onClick={() => setIsChartingOpen(true)}
            ...
            {/* Render AI Charting Modal */}
            {selectedClient && (
              <ChartingModal
                isOpen={isChartingOpen}
                onClose={() => setIsChartingOpen(false)}
                clientId={selectedClient.id}
                clientName={selectedClient.name}
                chartNumber={selectedClient.chart_number || (selectedClient.client_no ? `#${selectedClient.client_no}` : undefined)}
